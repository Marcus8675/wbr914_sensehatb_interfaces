...
{
	return lgI2cReadByteData(LPS22HB_fd, reg);
}

unsigned short I2C_readU16(int reg)
{
	return lgI2cReadWordData(LPS22HB_fd, reg);
}
void I2C_writeByte(int reg, int val)
{
	lgI2cWriteByteData(LPS22HB_fd, reg, val);
}
void LPS22HB_RESET()
{   unsigned char Buf;
...
{
    LPS22HB_fd = lgI2cOpen(1,LPS22HB_I2C_ADDRESS,0);
